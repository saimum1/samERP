...
const SearchDialouge = ({ setSearchValue, setFilterOpen ,setTableData, type }) => {
    const [selectedOption, setSelectedOption] = useState( type === 'offer'? 'Active' : 'Available');
    ...
    const handleSearch = async () => {
        try {
            ...
            const response = await axios.get(`${config.apiUrl}/api/${type}?status=${type === 'operator'?selectedOption.toLowerCase(): selectedOption}&operatorId=${selectedId}&entryDateTo=`);
        ...
        >
            ...
            >
                ...
                <select value={selectedOption}  name="status" onChange={(e) => setSelectedOption(e.target.value)} style={{border : '1px solid #595959'}} className="w-full cursor-pointer bg-[#404040] hover:bg-[#545454] text-[#9CA3AF]  py-2.5 px-4 rounded-md shadow-sm hover:shadow-md transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    <option value={type === 'offer'? 'Active' : 'Available'}>
                        {type === 'offer'? 'Active' : 'Available'}
                    </option>
                    <option value={type === 'offer'? 'Inactive' : 'Not Available'}>
                        {type === 'offer'? 'Inactive' : 'Not Available'}
                    </option>
                </select>
                </div>
                {type === 'sim' || type === 'offer'?<div style={{display : 'flex', alignItems : 'center', justifyContent : 'space-between',gap : '5%',}}>
